...
describe('Test ORManager chainInfo', () => {
  ...
  it(
    ...
    embedStorageVersionIncrease(
      ...
      async function () {
        ...
        for (let j = 0; j < 3; j++) {
          for (let i = 1; i <= 4; i++) {
            chainIds.push(Number(defaultChainInfoArray[i - 1].id));
            ...
            tokens.push({
              token: BigNumber.from(_token).add(0), // add(0), convert _hex uppercase to lowercase
        ...
        console.log(
          `length of chainIds: ${chainIds.length}, length of tokens: ${tokens.length}`,
        );
