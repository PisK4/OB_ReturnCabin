...
export const profitRootMock = ethers.utils.keccak256(
  ethers.utils.toUtf8Bytes('profitRoot'),
);
...
export const siblingsMock = ethers.utils.keccak256(
  ethers.utils.toUtf8Bytes('siblings'),
);
export const smtLeavesMock: SMTLeaf[] = [
  {
    chainId: 1,
    maker: ethers.Wallet.createRandom().address,
...
export function getRulesSetting(getNative: boolean) {
  ...
  const chain0MinPrice = BigNumber.from(5)
    .pow(parseInt(Math.random() * 18 + ''))
    .add(BigNumber.from('50000000000000000'));
  const chain0MaxPrice = BigNumber.from(5)
    .pow(parseInt(Math.random() * 19 + ''))
    .add(BigNumber.from('70000000000000000'));
  const chain1MinPrice = BigNumber.from(5)
    .pow(parseInt(Math.random() * 18 + ''))
    .add(BigNumber.from('50000000000000000'));
  const chain1MaxPrice = BigNumber.from(5)
    .pow(parseInt(Math.random() * 19 + ''))
    .add(BigNumber.from('80000000000000000'));
  const chain0withholdingFee = BigNumber.from(5)
    .pow(parseInt(Math.random() * 15 + ''))
    .add(BigNumber.from('100000000000000'));
  const chain1withholdingFee = BigNumber.from(5)
    .pow(parseInt(Math.random() * 15 + ''))
    .add(BigNumber.from('100000000000000'));
...
export async function mineXMinutes(time: number) {
  ...
  console.log(`mine ${time} minutes, current time: ${await getCurrentTime()}`);
}
